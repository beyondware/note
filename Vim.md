...
```sh
0（数字零）           移动到行首（相当于按【Home】键）
^（shift+数字6）      移动到行   非空白第一个字符
$                    移动到行尾（相当于按【End】键）
g_                   移动到行   非空白最后一个字符
```
...
```sh
Ctrl+F（forward：向前）  向下翻一页（相当于按【Page Down】键）
Ctrl+B（backward：向后） 向上翻一页（相当于按【Page Up】键）
...
```sh
f+搜索字符（find：查找）   行间搜索（;下一个  ,上一个）
F+搜索字符                反向搜索（与 f 相反）
j（jump：跳转）
t（till：直达）搜索字符的前一个字符
```
...
```sh
ct)                      删除到 ) 右括号里面的内容，并进入插入模式
ct"                      删除到 " 右双引号里面的内容，并进入插入模式
```
...
```sh
:% s/\<old\>/new/g  只替换 old（精准匹配），而不包括 xold 等
```
